...
if(!isset($_POST["getSent"]) OR $_POST["getSent"] != 1){
	...
	$query = "SELECT * FROM messages WHERE accountID = :toAccountID ORDER BY messageID DESC LIMIT 10 OFFSET $offset";
	$countquery = "SELECT count(*) FROM messages WHERE accountID = :toAccountID";
	$getSent = 1;
}
...
foreach ($result as &$message1) {
	if($message1["messageID"]!=""){
		...
		if($getSent == 1){
			...
			$accountID = $message1["accountID"];
		}
		$query=$db->prepare("SELECT * FROM accounts WHERE accountID = :accountID");
		$query->execute([':accountID' => $accountID]);
		$result12 = $query->fetchAll()[0];
		$msgstring .= "6:".$result12["userName"].":3:".$result12["accountID"].":2:".$result12["accountID"].":1:".$message1["messageID"].":4:".$message1["subject"].":8:".$message1["isNew"].":9:".$getSent.":7:".$uploadDate."|";
	}
}
