...
$query = "SELECT count(*) FROM blocks WHERE (person1 = :targetaccountid AND person2 = :me) OR (person2 = :targetaccountid AND person1 = :me)";
$query = $db->prepare($query);
$query->execute([':targetaccountid' => $targetaccountid, ':me' => $me]);
...
$query->execute();
/*$f = "SELECT rank FROM (
                  SELECT @rownum := @rownum + 1 AS rank, accountID
                  FROM users WHERE isBanned = '0' AND gameVersion > 19 AND stars > 25 ORDER BY stars DESC
                  ) as result WHERE accountID=:targetaccountid";*/
$f = "SELECT count(*) FROM accounts WHERE stars > :stars AND isBanned = 0"; //I can do this, since I already know the stars amount beforehand
$query = $db->prepare($f);
...
//var_dump($leaderboard);
	//accinfo
		$query = "SELECT youtubeurl, frS, mS FROM accounts WHERE accountID = :accountID";
		$query = $db->prepare($query);
		$query->execute([':accountID' => $targetaccountid]);
		$accinfo = $query->fetch();
		$reqsstate = $accinfo["frS"];
		$msgstate = $accinfo["mS"];
if($me==$targetaccountid){
	/* notifications */
		//friendreqs
	...
	//check if INCOING friend request
		$query = "SELECT ID,comment,uploadDate FROM friendreqs WHERE accountID = :targetaccountid AND toAccountID = :me";
		$query = $db->prepare($query);
		$query->execute([':targetaccountid' => $targetaccountid, ':me' => $me]);
	...
	//check if OUTCOMING friend request
		$query = "SELECT count(*) FROM friendreqs WHERE toAccountID = :targetaccountid AND accountID = :me";
		$query = $db->prepare($query);
		$query->execute([':targetaccountid' => $targetaccountid, ':me' => $me]);
	...
	//check if friend ALREADY
		$query = "SELECT count(*) FROM friendships WHERE (person1 = :me AND person2 = :targetaccountid) OR (person2 = :me AND person1 = :targetaccountid)";
		$query = $db->prepare($query);
		$query->execute([':me' => $me, ':targetaccountid' => $targetaccountid]);
...
echo "1:".$user["userName"].":2:".$user["accountID"].":13:".$user["coins"].":17:".$user["userCoins"].":10:".$user["color1"].":11:".$user["color2"].":3:".$user["stars"].":4:".$user["demons"].":8:".$creatorpoints.":18:".$msgstate.":19:".$reqsstate.":20:".$accinfo["youtubeurl"].":21:".$user["accIcon"].":22:".$user["accShip"].":23:".$user["accBall"].":24:".$user["accBird"].":25:".$user["accDart"].":26:".$user["accRobot"].":28:".$user["accGlow"].":30:".$rank.":16:".$user["accountID"].":31:".$friendstate.":29:1".$appendix;
