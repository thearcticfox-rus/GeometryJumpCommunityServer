...
$query = $db->prepare("SELECT accounts.accountID, accounts.userName, accounts.lastPlayed FROM accounts WHERE accountID IN ($accounts) ORDER BY userName ASC");
$query->execute();
...
foreach($result as &$action){
	...
	switch($action["type"]){
		...
		case 10:
			...
			break;
		case 11:
			$actionname = "Создал сборник уровней";
			break;
		default:
			$actionname = $action["type"];
			break;
		}
	if($action["type"] == 2 OR $action["type"] == 3 OR $action["type"] == 10){
		if($action["value"] == 1){
			$value = "True";
		}else{
			$value = "False";
	...
	if($action["type"] == 4 AND $action["value2"] > time()){
